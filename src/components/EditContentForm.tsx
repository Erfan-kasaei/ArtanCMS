...
type EditContentFormProps = {
  ...
  onUpdateContent: (updatedContent: { id: string; title: string; description?: string }) => void; // تابع بروزرسانی محتوا
};
...
export default function EditContentForm({
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    // بررسی پاسخ درخواست
    if (res.ok) {
      const updatedContent = await res.json(); // دریافت محتوای ویرایش شده از API
      onUpdateContent(updatedContent); // ارسال محتوای ویرایش شده به لیست
